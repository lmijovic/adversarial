...
def main (args):
    ...
    # LM this is for storing to root
    Xres = data[features]
    Yres = data['signal']
    Myyres =  data[features_decorrelation]
    lm_result = pd.concat([Xres,Yres,Myyres], axis=1, sort=False)
    ...
    aux_vars = ['logpt']
    data['logpt'] = pd.Series(np.log(data['y1_pt'].values), index=data.index)
    
    # Specify common weightsg
    # -- Classifier
    ...
    with Profile("Classifier-only fit, full"):
        ...
        if args.train or args.train_classifier:
            ...
            ## LM 
            class_pred = parallelised.predict(Xres)
            nall=Xres.shape[0]
            print("LM classifier predict",type(class_pred),len(class_pred),nall)
            df_class = pd.DataFrame(data=class_pred, index=range(0,nall), columns=['class_pred'])
            df_comb = pd.DataFrame(data=[-1 for i in range(0,nall)],index=range(0,nall), columns=['comb_pred'])

            
            lm_result = pd.concat([lm_result,df_class,df_comb], axis=1, sort=False)
            # saving analysis info to root file
            lm_result.to_root('out_class.root', tree_key='ANNTree')
    ...
    with Profile("Combined adversarial fit, full"):
        ...
        if args.train or args.train_adversarial:
            ...
            ## LM add  here
            ## LM 
            comb_pred = parallelised.predict(X)
            #print("LM combifier predict",type(comb_pred),len(comb_pred))
            #print("LM combifier predict0",type(comb_pred[0]),len(comb_pred[0]),comb_pred[0][0:4])
            #print("LM combifier predict1",type(comb_pred[1]),len(comb_pred[1]),comb_pred[0][0:4])
            df_comb = pd.DataFrame(data=comb_pred[0], index=range(0,len(comb_pred[0])), columns=['comb_pred'])
            #res_comb = pd.concat([X,Y,df_comb], axis=1, sort=False)
            print('X,Y:',type(X),len(X),type(Y),len(Y))
            print('X(0)',type(X[0]),len(X[0]),X[0][0:4])
            print('X(1)',type(X[1]),len(X[1]),X[1][0:4])
            print('X(2)',type(X[2]),len(X[2]),X[2][0:4])
            print('Y(0)',type(Y[0]),len(Y[0]),Y[0][0:4])
            print('Y(1)',type(Y[1]),len(Y[1]),Y[1][0:4])
            print('df_comb len', len(comb_pred[0]))
            #print(type(data),data.keys())
            df_class = pd.DataFrame(data=[-1 for i in range(0,len(comb_pred[0]))],index=range(0,len(comb_pred[0])), columns=['class_pred'])
            res_comb = pd.concat([data[features],data['signal'],data['myy'],df_comb,df_class], axis=1, sort=False)           
            res_comb.to_root('out_comb.root', tree_key='ANNTree')
        else:
